...
function saveModules() {
   ...
   if (userProjects[savePrInd].modules) {
      for (let j = 0; j < userProjects[savePrInd].modules.length; j++) {
         storedModules.push(userProjects[savePrInd].modules[j].name)
      }
      ...
      if (deletedModules.length != 0) {
         for (let del = 0; del < deletedModules.length; del++) {
            for (let j = 0; j < userProjects[savePrInd].modules.length; j++) {
               if (userProjects[savePrInd].modules[j].name == deletedModules[del]) {
                  userProjects[savePrInd].modules.splice(j,1)
               }
            }
         }
      }
      ...
      if (addedModules.length != 0) {
         for (let add = 0; add < addedModules.length; add++) {
            const modulesLength = userProjects[savePrInd].modules.length
            userProjects[savePrInd].modules[modulesLength] = {
   ...
   //If there're no modules in this project
      ...
      userProjects[savePrInd].modules = []
      for (let q = 0; q < modulesList.length; q++) {
         userProjects[savePrInd].modules[q] = {
...
function loadModules() {
   ...
   if (userProjects[prInd].modules) {
      for (let j = 0; j < userProjects[prInd].modules.length; j++) {
         addModule(userProjects[prInd].modules[j].name)
         let moduleToModify = document.querySelectorAll('.module')[document.querySelectorAll('.module').length-1]
         
         //Update scenarios counter
         if (userProjects[prInd].modules[j].scenarios) {
            moduleToModify.querySelector('.module-scenarios-counter').textContent = userProjects[prInd].modules[j].scenarios.length
         }
      }
   }
